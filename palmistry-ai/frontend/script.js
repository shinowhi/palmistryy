...
(() => {
  ...
  function renderResults(results, disclaimer, errors) {
    ...
    for (const key of ["sinh_dao", "su_nghiep", "tam_dao", "tri_dao"]) {
      ...
      // Ánh xạ nhan_raw từ advice_engine -> nhãn tiếng Việt thân thiện
      const NHAN_MAP = {
        "Dồi dào": "Dài",
        "Ổn định": "Trung Bình",
        "Cần bồi dưỡng": "Ngắn",
        "Bứt phá": "Dài",
        "Tiến triển đều": "Trung Bình",
        "Đang tìm hướng": "Ngắn",
        "Sâu sắc": "Dài",
        "Hài hòa": "Trung Bình",
        "Cần lắng nghe bản thân": "Ngắn",
        "Vượt trội": "Dài",
        "Tư duy ổn định": "Trung Bình",
        "Cần thời gian tập trung": "Ngắn",
      };
      const nhanRaw = data.nhan || "";
      const nhanNgan = NHAN_MAP[nhanRaw] || nhanRaw;
 
      // Cập nhật badge class theo nhận định để đổi màu
      if (badgeEl) {
        badgeEl.classList.remove("is-dai", "is-trungbinh", "is-ngan");
        if (nhanNgan === "Dài") badgeEl.classList.add("is-dai");
        else if (nhanNgan === "Ngắn") badgeEl.classList.add("is-ngan");
